...
from metrics.loss import FocalLossMultiClass, DiceLoss
...
class ClassifierSegExperimentCV:
    """
    ...
    def __init__(self, args, train_loader, val_loader, n_classes, checkpoint_file, Network, fold_path, fold_num, c_weights=None):
        ...
        self.upscale_factor     = 8     # upsample ratio for attention visualization
        ...
        dice_loss = DiceLoss()
        self.dice_loss_function = dice_loss.to(self.device)
    ...
    def train(self):
        ...
        for batch_idx, (data, masks, target) in enumerate(self.train_loader):
            # add a dimension, from (32, 224, 224) to (32, 1, 224, 224) to match the attention map
            masks = masks.unsqueeze(1) 

            # adapts the spatial dimensions of the input tensor to a specified output size, 
            # performing average pooling to obtain the output.
            seg_1 = F.adaptive_avg_pool2d(masks, self.img_size//self.upscale_factor)
            seg_2 = F.adaptive_avg_pool2d(masks, self.img_size//self.upscale_factor//2)

            data, seg_1, seg_2, target =  \
                data.to(self.device), seg_1.to(self.device), seg_2.to(self.device), target.to(self.device)

            # Zero the gradients
            self.optimizer.zero_grad()

            # Forward pass
            output, att1,  att2 = self.model(data)

            # Compute the loss
            loss = self.loss_function(output, target)

            # get the loss from the attention maps and segmmentation masks
            loss_seg1 = self.dice_loss_function(att1, seg_1)
            loss_seg2 = self.dice_loss_function(att2, seg_2)

            # combine the losses
            loss = loss + 0.001 * loss_seg1 + 0.01 * loss_seg2
